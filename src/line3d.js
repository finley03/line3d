	...
	class Objects {
		...
		createButtons() {
			for (let shape in this.shapes) {
				...
				button.type = "button";
				button.innerHTML = this.shapes[shape].fancyname; //allows for spaces in names
				button.className = "objbutton"; // must be identified by class, see the difference between IDs and classes. http://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/id
...
// add window resize listener
	...
	if(window.location.hash.substr(1)){
		// use fragment identifier to select shape, makes it so i dont have to click button every time.
